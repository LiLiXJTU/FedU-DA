...
import torchvision
#from Load_Dataset import RandomGenerator,ValGenerator
from torch.utils.data import Dataset, TensorDataset, ConcatDataset
from torchvision import datasets, transforms

from src.Load_Dataset import ImageToImage2D
...
def init_net(model, init_type, init_gain, gpu_ids=0):
    """Function for initializing network weights.
...
def create_datasets(data_path, dataset_name, num_clients, num_shards, iid):
    ...
    if hasattr(torchvision.datasets, dataset_name):
        ...
        # train_dataset = './data/UESTC_zhuanjia/Train_Folder/'
        # test_dataset = './data/UESTC_zhuanjia/Test_Folder'
        # img_size = 224
        # train_tf = transforms.Compose([RandomGenerator(output_size=[img_size, img_size])])
        # test_tf = ValGenerator(output_size=[img_size, img_size])
        # training_dataset = ImageToImage2D(train_dataset, train_tf, image_size=img_size)
        # test_dataset = ImageToImage2D(test_dataset, test_tf, image_size=img_size)
    else:
...
def save_on_batch(images, masks, pred, names, vis_path):
    '''Computes the mean Area Under ROC Curve over a batch during training'''
    for i in range(pred.shape[0]):
        # pred = np.argmax(pred[i], axis=-1).astype('uint8')
        # pred = np.expand_dims(pred, axis=-1)
        # pred = np.expand_dims(np.argmax(pred[0].cpu().detach().numpy(), axis=0).astype('uint8'), axis=0)

        #pred_tmp = pred[i][0].cpu().detach().numpy()
        mask = np.argmax(pred[i].cpu().detach().numpy(), axis=0)
        mask_tmp = masks[i].cpu().detach().numpy()
        #test = torch.max(pred[i],dim=0).indices
        pred_tmp = torch.ones_like(torch.as_tensor(mask)).numpy()
        for k in range(len(pred_tmp[1])):
            for j in range(len(pred_tmp[0])):
                if mask[k][j] == 1:
                    pred_tmp[k][j] = 255
                if mask[k][j] == 0:
                    pred_tmp[k][j] = 0

        # pred_tmp[mask_tmp>0] = 255
        # pred_tmp[mask_tmp<=0] = 0
